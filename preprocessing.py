...
def create_data_set(url_lst, path_to_data_set, mode='phishing'):
    with open(url_lst) as urls:
        with open(path_to_data_set + 'dataset.csv', 'a') as csv_file:
            writer = csv.writer(csv_file)
            for url in urls:
                if url == '' or url == '\n':
                    continue
                else:
                    entry = extract_features(url)
                    if mode == 'phishing':
                        entry.append(-1)
                    elif mode == 'benign':
                        entry.append(1)
...
def extract_features(url):
    ...
    lst = [x.start(0) for x in re.finditer('//', url)]
    if list[len(lst)-1] > 7:
        data_set.append(-1)
    else:
        data_set.append(1)
    ...
    if soup == -999:
        ...
        # 16. SFH
        sfh = soup.find_all('form', action= True)
        if len(sfh) == 0:
            data_set.append(1)
        else:
            for form in sfh:
